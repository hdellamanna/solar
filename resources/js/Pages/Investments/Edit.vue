...
const form = useForm({
    ...
    quantity:      String(props.investment.quantity),
    average_price: String(props.investment.average_price_decimal),
    current_price: props.investment.current_price_decimal !== null && props.investment.current_price_decimal !== undefined
        ? String(props.investment.current_price_decimal)
        : '',
    currency:      props.investment.currency,
    acquired_at:   props.investment.acquired_at ?? '',
...
<template>
    ...
    <AuthenticatedLayout title="Editar investimento">
        <div class="max-w-2xl">
            <form @submit.prevent="submit" class="card-elevated p-6 md:p-8 space-y-6">
                <div class="flex items-center gap-3 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50">
                    ...
                    <div class="min-w-0">
                        <p class="font-semibold tracking-tight truncate">{{ form.name || 'Editar investimento' }}</p>
                ...
                <div>
                    ...
                    <input v-model="form.name" type="text" maxlength="100" placeholder="Ex: Tesouro Selic 2029, ITSA4, Bitcoin..." class="input" :class="form.errors.name ? 'border-expense' : ''" required autofocus />
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-5 gap-1 p-1 rounded-2xl bg-slate-100 dark:bg-slate-800/60">
                        <button v-for="opt in typeOptions" :key="opt.value" type="button" @click="form.type = opt.value" :class="['px-2 py-2 rounded-xl text-xs font-medium transition-all duration-200 ease-out text-center', form.type === opt.value ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white']">{{ opt.label }}</button>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input v-model="form.ticker" type="text" maxlength="20" placeholder="ITSA4, HASH11, BTC..." class="input font-mono uppercase" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input v-model="form.acquired_at" type="date" class="input tabular-nums" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input v-model="form.quantity" type="number" step="0.00000001" min="0" placeholder="100" class="input flex-1 tabular-nums text-lg font-semibold" :class="form.errors.quantity ? 'border-expense' : ''" required />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input v-model="form.average_price" type="number" step="0.01" min="0" placeholder="0,00" class="input pl-10 tabular-nums text-lg font-semibold" :class="form.errors.average_price ? 'border-expense' : ''" required />
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-slate-500">{{ form.currency === 'USD' ? '$' : form.currency === 'EUR' ? '€' : form.currency === 'GBP' ? '£' : 'R$' }}</span>
                            <input v-model="form.current_price" type="number" step="0.01" min="0" placeholder="0,00" class="input pl-10 tabular-nums text-lg font-semibold" :class="form.errors.current_price ? 'border-expense' : ''" />
                        </div>
                ...
                <div>
                    ...
                    <textarea v-model="form.notes" rows="2" maxlength="1000" placeholder="Estratégia, corretora, observações..." class="input resize-none"></textarea>
                </div>
