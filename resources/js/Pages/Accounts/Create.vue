...
const form = useForm({
    ...
    balances: props.account?.balances
        ? props.account.balances.map(b => ({ currency: b.currency, balance_cents: b.balance_cents / 100 }))
        : [],
});
...
const submit = () => {
    // FASE 6A: send balances as cents to the backend.
    ...
    if (props.account) {
        ...
        form.post(route('accounts.store'), { data: payload });
    }
};
</script>
        ...
        <div class="max-w-2xl">
            <form @submit.prevent="submit" class="card-elevated p-6 md:p-8 space-y-5">
                <div>
                    ...
                    <input v-model="form.name" type="text" placeholder="Ex: Nubank, Itaú, Wise, Nomad Global" class="input" :class="form.errors.name ? 'border-expense' : ''" required>
                ...
                <!-- Single-currency balance (default flow) -->
                <div v-if="!isMulti">
